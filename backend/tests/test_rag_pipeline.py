        ...
        chunks = [
            {
                ...
                "metadata": {"filename": "document1.pdf", "page": 1},
            },
            {
                "content": "This is the second document content.",
                "metadata": {"filename": "document2.pdf", "page": 3},
            },
        ]
...
class TestGenerateAnswer:
    ...
    async def test_generate_answer_success(self, mock_config):
        ...
        mock_response = AIMessage(
            content="This is a test answer with citation [doc.pdf p. 1]."
        )
    ...
    async def test_generate_answer_rate_limit_exhausted(self, mock_config):
        ...
        mock_llm = AsyncMock()
        mock_llm.ainvoke.side_effect = RateLimitError(
            "Rate limit exceeded", response=create_mock_response(), body=None
        )
        mock_config.llm = mock_llm
    ...
    async def test_generate_answer_connection_error(self, mock_config):
        ...
        mock_llm = AsyncMock()
        mock_llm.ainvoke.side_effect = APIConnectionError(
    ...
    async def test_generate_answer_api_error_server(self, mock_config):
        ...
        server_error = APIError(
            "Server error", request=create_mock_request(), body=None
        )
...
class TestAnswerQuestion:
    ...
    @pytest.mark.anyio
    async def test_answer_question_integration(
        self, mock_extract, mock_generate, mock_search
    ):
        ...
        mock_search.assert_called_once_with(
            "What is the test?", 123, k=5
        )  # Default limit of 5
        mock_generate.assert_called_once()
        mock_extract.assert_called_once_with(
            "Test answer with [test.pdf p. 1]", mock_chunks
        )
    ...
    @patch("app.core.rag_pipeline.extract_citations")
    @pytest.mark.anyio
    async def test_answer_question_with_custom_limit(
        self, mock_extract, mock_generate, mock_search
    ):
    ...
    @patch("app.core.rag_pipeline.extract_citations")
    @pytest.mark.anyio
    async def test_answer_question_with_document_ids_filter(
        self, mock_extract, mock_generate, mock_search
    ):
        ...
        mock_chunks = [
            ...
            },
            {
                "content": "Content 2",
                "document_id": 2,
                "metadata": {"filename": "doc2.pdf", "page": 1},
            },
            {
                "content": "Content 3",
                "document_id": 3,
                "metadata": {"filename": "doc3.pdf", "page": 1},
            },
        ]
        mock_search.return_value = mock_chunks
        mock_generate.return_value = "Test answer"
    ...
    @patch("app.core.rag_pipeline.extract_citations")
    @pytest.mark.anyio
    async def test_answer_question_with_both_limit_and_document_ids(
        self, mock_extract, mock_generate, mock_search
    ):
    ...
    @patch("app.core.rag_pipeline.extract_citations")
    @pytest.mark.anyio
    async def test_answer_question_invalid_document_ids(
        self, mock_extract, mock_generate, mock_search
    ):
        ...
        await answer_question(
            "What is the test?", 123, document_ids=["invalid", "also_invalid"]
        )
...
class TestExtractCitations:
    ...
    def test_extract_citations_alternative_page_formats(self):
        ...
        chunks = [
            {"content": "Doc content", "metadata": {"filename": "doc.pdf", "page": 5}},
            {
                "content": "File content",
    ...
    def test_extract_citations_snippet_creation(self):
        ...
        long_content = (
            "This is a very long piece of content that should be truncated when creating snippets for citations. "
            * 10
        )
        answer = "According to [doc.pdf p. 1], this is true."
        chunks = [
            {"content": long_content, "metadata": {"filename": "doc.pdf", "page": 1}}
        ]
    ...
    def test_extract_citations_document_id_generation(self):
        ...
        chunks = [
            ...
            {"content": "Content without page", "metadata": {"filename": "nodoc.pdf"}},
        ]
