...
class ErrorHandler:
    ...
    @classmethod
    def _log_error(
        cls, error: Exception, request_id: str, status_code: int, logger: logging.Logger
    ) -> None:
        ...
        if status_code >= 500:
            ...
            logger.warning(
                f"Client error in RAG pipeline: {str(error)}", extra=log_data
            )
        else:
...
class FallbackResponseManager:
    ...
    @classmethod
    def get_fallback_response(cls, error: Exception) -> dict[str, Any] | None:
        """
